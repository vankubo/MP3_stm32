...
void DMA1_NVICcfg(IRQn_Type DMA1_irq,uint8_t PreemptionPriority,uint8_t SubPriority)
{
	...
	NVIC_InitStructure.NVIC_IRQChannel = DMA1_irq;			//使能中断通道DMA1_ChannelX_IRQn
...
{ 
	DMA_Cmd(DMA_CHx, DISABLE );  //关闭DMA_CHx DMA1 所指示的通道      
 	...
 	DMA_Cmd(DMA_CHx, ENABLE);  //使能DMA_CHx DMA1 所指示的通道 
}	  
...
void DMA1_Channel3_IRQHandler(void)
{
	...
	if(DMA_GetITStatus(DMA1_IT_TC3))
    {
			...
			if(DMA_GetFlagStatus(DMA1_FLAG_TC3)!=RESET)//判断通道4传输完成*不知道什么原因，目前只能用这种方式
			{
